...
export default function Transaction() {
    ...
    useEffect(() => {
        axios({
            ...
            setFilteredTransactions(res.data.body || []);
            setLoading(false);
            localStorage.setItem("transaction", JSON.stringify(res.data.body));
        })
            .catch((error) => {
    ...
    const handleDelete = (tId) => {
        axios({
            ...
            setTransactions((prevTransactions) =>
                prevTransactions.filter((trans) => trans.tId !== tId)
            );
        }).catch(error => {
    ...
    const applyFilter = () => {
        const filtered = transactions.filter((trans) => {
            return Object.keys(filters).every((column) => {
                ...
                if (column === 'sourceAccId' && !trans.sourceAccId.toString().includes(filterValue)) return false;
                if (column === 'transacDecription' && !trans.transacDecription.toString().includes(filterValue)) return false;
                if (column === 'transacAmount' && !trans.transacAmount.toString().includes(filterValue)) return false;
                if (column === 'transacTime' && !trans.transacTime.toString().includes(filterValue)) return false;
    ...
    const handleViewTransaction = (tId) => {
        setTimeout(() => navigate(`/viewtransaction/${tId}`), 2000);
    };
    ...
    const exportToPDF = () => {
        ...
        const tableRows = filteredTransactions.map((transaction) => [transaction.sourceAccId, transaction.transacDecription, transaction.transacAmount, transaction.transacTime]);
        doc.autoTable({
            head: [tableColumns],
            body: tableRows,
            startY: 20,
                            ...
                            filteredTransactions.length === 0 ? (
                                <tr>
                                    <td colSpan="4" className="text-center text-gray-500 py-4 text-xl font-semibold">
                                        No Transactions Available<span className="text-xl">❗</span>
                                    </td>
                                </tr>
                            ) : (
